#!/bin/bash
################################################################################
# Setup Script for Manualmode Server Environment Configuration
...
echo -e "${GREEN}╔═══════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  Manualmode Server Environment Setup                   ║${NC}"
...
cat > .env.summary << EOF
Manualmode Server Configuration Summary
Generated on $(date)
...
if [ "$SETUP_STAGING" = true ]; then
    create_env_file "staging" "Staging" "meaningful_conversations_staging" "http://91.99.193.87:8080"
fi

# Create production environment
if [ "$SETUP_PRODUCTION" = true ]; then
    create_env_file "production" "Production" "meaningful_conversations_production" "http://91.99.193.87"
fi
...
echo ""
echo -e "2. ${BLUE}Deploy to manualmode server:${NC}"
if [ "$SETUP_STAGING" = true ]; then
    echo -e "   make deploy-manualmode-staging"
fi
if [ "$SETUP_PRODUCTION" = true ]; then
    echo -e "   make deploy-manualmode-production"
fi
echo ""
echo -e "3. ${BLUE}Monitor deployment:${NC}"
if [ "$SETUP_STAGING" = true ]; then
    echo -e "   make logs-manualmode-staging"
fi
if [ "$SETUP_PRODUCTION" = true ]; then
    echo -e "   make logs-manualmode-production"
fi
echo ""
...
# Cleanup note for old .env files from alternative server
if [ -f .env.alternative ] || [ -f .env.staging ] || [ -f .env.production ]; then
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    ...
    echo -e "${YELLOW}  - New server: 91.99.193.87 (manualmode)${NC}"
