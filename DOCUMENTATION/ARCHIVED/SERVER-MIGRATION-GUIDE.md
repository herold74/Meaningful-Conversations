...
# Create directories
mkdir -p /opt/manualmode-staging
mkdir -p /opt/manualmode-production
...
tar -czf /tmp/mc-migration/compose-files.tar.gz \
  /opt/manualmode-staging/podman-compose.yml \
  /opt/manualmode-production/podman-compose.yml
...
# Verify files
ls -la /opt/manualmode-staging/.env
ls -la /opt/manualmode-production/.env
...
# Update deploy-manualmode.sh with new IP
# (See section below for script modifications)

# Deploy staging environment
./deploy-manualmode.sh --environment staging --server <NEW_SERVER_IP>

# Deploy production environment
./deploy-manualmode.sh --environment production --server <NEW_SERVER_IP>
```
...
## 🔧 Modify deploy-manualmode.sh for New Server

Update the script to use the new server IP:

```bash
# Edit line 17 in deploy-manualmode.sh:
...
```bash
# Check old server:
ssh root@<YOUR_SERVER_IP> 'cat /opt/manualmode-staging/.env | grep MARIADB_ROOT_PASSWORD'
```
...
# Stop pods
podman-compose -f /opt/manualmode-staging/podman-compose.yml down
podman-compose -f /opt/manualmode-production/podman-compose.yml down
...
cat /opt/manualmode-staging/.env | grep MARIADB
```
