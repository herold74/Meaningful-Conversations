# 🚀 Manualmode Server: Complete Deployment Guide

Complete guide for deploying Meaningful Conversations to your manualmode server (`<YOUR_SERVER_IP>`) using **Podman with dual environments (Staging & Production)**, each with its own isolated database and pod.
...
└── deploy-manualmode.sh             # Deployment script (supports both)
```
...
# Or specific version
./deploy-manualmode.sh -e production -v 1.4.8 --skip-build
```
...
./deploy-manualmode.sh -e production -v 1.4.7 --skip-build
```
