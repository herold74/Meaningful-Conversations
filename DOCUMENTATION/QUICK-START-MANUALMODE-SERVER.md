...
# SSH and restart
ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-staging && podman-compose -f podman-compose-staging.yml restart'
```
...
# SSH and restart
ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-production && podman-compose -f podman-compose-production.yml restart'
```
...
| `deploy-manualmode.sh` | Deployment script (supports both envs) | ✅ Yes |
...
ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-staging && podman-compose -f podman-compose-staging.yml ps mariadb'

# Check database logs
ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-staging && podman-compose -f podman-compose-staging.yml logs mariadb'
...
```bash
# Check if database is running
ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-production && podman-compose -f podman-compose-production.yml ps mariadb'

# Check database logs
ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-production && podman-compose -f podman-compose-production.yml logs mariadb'
...
# Restart backend
ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-staging && podman-compose -f podman-compose-staging.yml restart backend'
```
...
# Restart backend
ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-production && podman-compose -f podman-compose-production.yml restart backend'
```
...
ssh root@<YOUR_SERVER_IP> 'netstat -tulpn | grep -E ":80 |:443 "'
...
When you run `make deploy-manualmode-staging` or `make deploy-manualmode-production`, here's what happens:
...
```bash
# Staging aliases
alias deploy-staging="cd /path/to/project && make deploy-manualmode-staging"
alias logs-staging="ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-staging && podman-compose -f podman-compose-staging.yml logs -f'"
alias status-staging="ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-staging && podman-compose -f podman-compose-staging.yml ps'"

# Production aliases
alias deploy-prod="cd /path/to/project && make deploy-manualmode-production"
alias logs-prod="ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-production && podman-compose -f podman-compose-production.yml logs -f'"
alias status-prod="ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-production && podman-compose -f podman-compose-production.yml ps'"
```
...
watch -n 5 "ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-staging && podman-compose -f podman-compose-staging.yml ps'"

# Tab 3: Check resources
ssh root@<YOUR_SERVER_IP> "podman pod stats meaningful-conversations-staging"
