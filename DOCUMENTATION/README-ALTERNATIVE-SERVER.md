...
Deploy Meaningful Conversations to **`ssh root@46.224.37.130`** using Podman + Quay Registry!
...
# Access: http://46.224.37.130:8080

# 3️⃣ Deploy to production
make deploy-alternative-production
# Access: http://46.224.37.130
```
...
deploy-alternative.sh           # Deployment script (supports both envs)
setup-alternative-env.sh        # Legacy setup wizard
```
...
```bash
# Deployment
make deploy-alternative-staging               # Deploy all to staging
make deploy-alternative-production            # Deploy all to production
make deploy-alternative-staging-frontend      # Frontend to staging
make deploy-alternative-production-frontend   # Frontend to production
