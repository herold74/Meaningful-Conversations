...
- **Cloud Provider:** Hetzner VPS (<YOUR_SERVER_IP>)
...
ssh root@<YOUR_SERVER_IP> "sed -i 's/VERSION=.*/VERSION=1.7.7/' /opt/manualmode-staging/.env"
...
ssh root@<YOUR_SERVER_IP> 'bash /opt/manualmode-production/update-nginx-ips.sh staging'
```

**2. Production-Deployment:** Erst nach erfolgreichem Staging-Test!
```bash
# Analog zu Staging, aber mit production statt staging
ssh root@<YOUR_SERVER_IP> 'bash /opt/manualmode-production/update-nginx-ips.sh production'
```
...
```bash
ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-staging && podman-compose -f podman-compose-staging.yml exec backend npx prisma migrate status'
```
