...
- **Staging deploy (2026-07-24):** Build **2** — health OK
- **Production deploy (2026-07-24):** `./deploy-manualmode.sh -e production -c app` — v2.1.0 Build **2**, GitLab registry on prod, health OK; Web/PWA users see full visual redesign immediately
...
- **Backend:** `routes/gemini/chat.js` — `skipMistralBehaviorRules` when `isPreSeededTopic`. `bots.js` — Bekky handoff without stage directions; Dan EN/DE session-opening variant pools + newcomer hint. `services/dynamicPromptController.js` — restricted adaptive injection for `dan-client-language` (pacing/courtesy only).
...
### Bekky thought-audit upgrades + referral handoff + Dan (client exact language)
- **Backend:** \`meaningful-conversations-backend/bots.js\` — Bekky EN/DE: Thought Audit Log continuity prompt, pace calibration, thought-type routing with trailing \`[REFERRAL:…]\`, flexible turnaround examples (1–2), Phase 4 specificity + optional Life Context step + paired \`[AUDIT_TASK]…[/AUDIT_TASK]\`; DE duplicate header removed; **Dan** (\`dan-client-language\`, client exact language EN/DE, referral to Bekky, optional meditation markers) — **\`accessTier: 'client'\`** (nur Klienten / Freischaltung / Admin).
