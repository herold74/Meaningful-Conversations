...
- **Backend:** \`meaningful-conversations-backend/bots.js\` — Bekky EN/DE: Thought Audit Log continuity prompt, pace calibration, thought-type routing with trailing \`[REFERRAL:…]\`, flexible turnaround examples (1–2), Phase 4 specificity + optional Life Context step + paired \`[AUDIT_TASK]…[/AUDIT_TASK]\`; DE duplicate header removed; new premium bot **Dan** (\`dan-client-language\`, client exact language EN/DE, referral to Bekky, optional meditation markers).
