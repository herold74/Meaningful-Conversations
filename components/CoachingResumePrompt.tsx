...
import type { CoachingSessionDraft } from '../utils/coachingSessionDraft';
import { RotateCcw, X } from 'lucide-react';

interface CoachingResumePromptProps {
  draft: CoachingSessionDraft;
  onResume: () => void;
  onDiscard: () => void;
}

const CoachingResumePrompt: React.FC<CoachingResumePromptProps> = ({
  draft,
  onResume,
  onDiscard,
}) => {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="coaching-draft-resume-title"
    >
      <div className="surface-elevated w-full max-w-md rounded-2xl border border-border-primary p-6 shadow-xl">
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <h2
              id="coaching-draft-resume-title"
              className="text-lg font-bold text-content-primary"
            >
              {t('coaching_draft_resume_title')}
            </h2>
          </div>
          <button
            type="button"
            ...
            aria-label={t('coaching_draft_discard_button')}
          >
            <X className="w-5 h-5" aria-hidden />
          </button>
        </div>
        <p className="text-sm text-content-secondary mb-6">
          {t('coaching_draft_resume_body', {
            coach: draft.bot.name,
            count: messageCount,
          })}
        </p>
        <p className="text-xs text-content-secondary mb-6">
          {t('coaching_draft_resume_privacy_note')}
        </p>
        <div className="flex flex-col-reverse sm:flex-row gap-2 sm:justify-end">
          ...
          >
            {t('coaching_draft_discard_button')}
          </button>
          <button
            type="button"
            onClick={onResume}
          ...
          >
            {t('coaching_draft_resume_button')}
          </button>
        </div>
      </div>
    </div>
...
export default CoachingResumePrompt;
