...
const ChatView: React.FC<ChatViewProps> = ({ bot, lifeContext, chatHistory, setChatHistory, onEndSession, onMessageSent, currentUser, isNewSession, experimentalMode, encryptionKey }) => {
  ...
  useEffect(() => {
    console.log('🔍 ChatView Debug:', {
      botId: bot.id,
      experimentalMode,
      shouldShowBadge: experimentalMode && experimentalMode !== 'OFF' && bot.id === 'chloe-structured-reflection'
    });
  ...
  useEffect(() => {
    const checkVoiceAvailability = async () => {
      ...
      const getBestServerVoice = (botId: string, lang: string): string | null => {
        ...
        if (lang === 'en') {
          const maleBotsEN = ['max-ambitious', 'rob-pq', 'kenji-resilience', 'nexus-goal-path-solution'];
  ...
  const speak = useCallback(async (text: string, isMeditation: boolean = false) => {
    ...
    if (isMeditation && (bot.id === 'rob-pq' || bot.id === 'kenji-resilience')) {
        utterance.rate = 0.9;
    }
      ...
      try {
        ...
        if (healthData.status === 'ok' && healthData.piperAvailable) {
          ...
          const getBestServerVoice = (botId: string, lang: string): string | null => {
            ...
            if (lang === 'en') {
              const maleBotsEN = ['max-ambitious', 'rob-pq', 'kenji-resilience', 'nexus-goal-path-solution', 'victor-systemic-coaching'];
              gender = maleBotsEN.includes(botId) ? 'male' : 'female';
            } else if (lang === 'de') {
              const femaleBotsDE = ['g-interviewer', 'ava-strategic', 'chloe-structured-reflection'];
              gender = femaleBotsDE.includes(botId) ? 'female' : 'male';
