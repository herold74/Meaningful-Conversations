...
- Coach-Practice-Auswertungen (inkl. Gesprächstranskript sofern gespeichert — löschbar in der Auswertungsansicht) und Transkript-Auswertungen (falls vorhanden)
...
- **Coach Practice (abgeschlossen):** Nach freiwilliger Auswertung speichern wir die Bewertungsergebnisse und das **Gesprächstranskript** auf unserem Server (nicht Ende-zu-Ende-verschlüsselt — bleibt bei Passwortänderung oder -vergessen erhalten). Der Übungs-Klient ist eine KI-Simulation. Sie können das Transkript jederzeit in der Auswertungsansicht löschen; die Bewertung bleibt. Bei Kontolöschung werden alle Übungsdaten entfernt.

Die übrigen Einträge oben (Authentifizierung, Sprache, Merken der E-Mail, Gastmodus, Coach Practice Entwurf) werden **lokal** in Ihrem Browser gespeichert. Sie können diese lokalen Daten jederzeit in Ihren Browser-Einstellungen löschen.
...
Im Sinne der Transparenzpflichten der Verordnung (EU) 2024/1689 (AI Act) weisen wir ausdrücklich darauf hin:

- Sie interagieren in dieser Anwendung **ausschließlich mit KI-Systemen** — nicht mit menschlichen Coaches, Therapeut:innen oder Berater:innen. Avatare und Namen dienen der Orientierung.
- Antworten werden von automatisierten Sprachmodellen (**Google Gemini** und/oder **Mistral AI**) generiert und zur Verarbeitung an den jeweils konfigurierten Dienst übermittelt (siehe Abschnitt 5.1).
- Es findet **keine biometrische Emotionserkennung** statt (keine Auswertung von Kamera-, Video- oder Stimmprofilen zur Erkennung von Emotionen).
- Es werden **keine automatisierten Entscheidungen** über Sie getroffen, die rechtliche oder vergleichbar erhebliche Folgen hätten.
- Sie können fehlerhafte KI-Antworten im Chat melden und Feedback geben; bei Fragen: support@manualmode.at.

Weitere Hinweise zum Bildungscharakter und zur Haftung finden Sie im Haftungsausschluss und im Benutzerhandbuch.
...
- Coach practice evaluations (including session transcript if stored — deletable from the evaluation review) and transcript evaluations (if any)
...
- **Coach Practice (completed):** After you voluntarily finish an evaluation, we store the evaluation results and **session transcript** on our server (not end-to-end encrypted — survives password change or reset). The practice coachee is an AI simulation. You can delete the transcript anytime from the evaluation review; scores remain. All practice data is removed when you delete your account.

The other items above (authentication, language, remember email, guest mode, Coach Practice draft) are stored **locally** in your browser. You can delete that local data anytime in your browser settings.
...
In accordance with the transparency obligations of Regulation (EU) 2024/1689 (AI Act), we expressly inform you that:

- In this application, you interact **exclusively with AI systems** — not with human coaches, therapists, or advisors. Avatars and names help you navigate.
- Responses are generated by automated language models (**Google Gemini** and/or **Mistral AI**), and conversation content is sent to the configured service for processing (see Section 5.1).
- The app performs **no biometric emotion recognition** (no analysis of camera, video, or voice profiles to detect emotions).
- **No automated decisions** about you are made that produce legal or similarly significant consequences.
- You can report problematic AI responses in chat and submit feedback; questions: support@manualmode.at.

Further information on the educational purpose and liability is available in the Disclaimer and User Guide.
