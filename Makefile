...
db-backup-staging: ## Backup MariaDB on staging
	...
	@ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-staging && podman-compose -f podman-compose-staging.yml exec -T mariadb mysqldump -u root -p$${DB_ROOT_PASSWORD} meaningful_conversations_staging' > backup-staging-$$(date +%Y%m%d-%H%M%S).sql
	@echo "Backup saved to: backup-staging-$$(date +%Y%m%d-%H%M%S).sql"

db-backup-production: ## Backup MariaDB on production
	@echo "Backing up production database..."
	@ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-production && podman-compose -f podman-compose-production.yml exec -T mariadb mysqldump -u root -p$${DB_ROOT_PASSWORD} meaningful_conversations_production' > backup-production-$$(date +%Y%m%d-%H%M%S).sql
