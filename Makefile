...
db-shell-manualmode-staging: ## Open MariaDB shell on manualmode staging
	@ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-staging && podman-compose -f podman-compose-staging.yml exec mariadb mysql -u mcuser -p meaningful_conversations_staging'

db-shell-manualmode-production: ## Open MariaDB shell on manualmode production
	@ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-production && podman-compose -f podman-compose-production.yml exec mariadb mysql -u mcuser -p meaningful_conversations_production'

db-backup-manualmode-staging: ## Backup MariaDB on manualmode staging
	@echo "Backing up staging database..."
	@ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-staging && podman-compose -f podman-compose-staging.yml exec -T mariadb mysqldump -u root -p$${DB_ROOT_PASSWORD} meaningful_conversations_staging' > backup-manualmode-staging-$$(date +%Y%m%d-%H%M%S).sql
...
db-backup-manualmode-production: ## Backup MariaDB on manualmode production
	...
	@ssh root@<YOUR_SERVER_IP> 'cd /opt/manualmode-production && podman-compose -f podman-compose-production.yml exec -T mariadb mysqldump -u root -p$${DB_ROOT_PASSWORD} meaningful_conversations_production' > backup-manualmode-production-$$(date +%Y%m%d-%H%M%S).sql
