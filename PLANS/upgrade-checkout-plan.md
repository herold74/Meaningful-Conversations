...
| `KENJI_UNLOCK` | Kenji Coach Unlock | €3.90 | Permanent | `unlockedCoaches += kenji-resilience` |
| `CHLOE_UNLOCK` | Chloe Coach Unlock | €3.90 | Permanent | `unlockedCoaches += chloe-structured-reflection` |
...
function calculatePrice(user, productId) {
  ...
  if (baseProduct.category === 'premium') {
    const ownedBots = JSON.parse(user.unlockedCoaches || '[]')
      .filter(b => ['kenji-resilience', 'chloe-structured-reflection'].includes(b));
