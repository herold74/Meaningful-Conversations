...
- Access to staging server: `ssh root@<YOUR_SERVER_IP>`
...
ssh root@<YOUR_SERVER_IP> "cd /opt/manualmode-staging && podman ps | grep frontend"
...
```bash
# Check containers
ssh root@<YOUR_SERVER_IP> "podman ps | grep frontend"

# Test Blue (should still work)
curl -I https://mc-beta.manualmode.at

# Test Green directly
ssh root@<YOUR_SERVER_IP> "curl -I http://localhost:8083"
```
