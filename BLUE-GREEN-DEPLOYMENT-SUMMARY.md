...
ssh root@<YOUR_SERVER_IP> "/usr/local/bin/update-nginx-blue-green.sh disable-green"
ssh root@<YOUR_SERVER_IP> "podman stop meaningful-conversations-frontend-green"
...
scp nginx-config/blue-green-routing.conf root@<YOUR_SERVER_IP>:/etc/nginx/sites-available/meaningful-conversations-production.conf
ssh root@<YOUR_SERVER_IP> "ln -sf /etc/nginx/sites-available/meaningful-conversations-production.conf /etc/nginx/sites-enabled/"

# 2. Copy management script
scp nginx-config/update-nginx-blue-green.sh root@<YOUR_SERVER_IP>:/usr/local/bin/
ssh root@<YOUR_SERVER_IP> "chmod +x /usr/local/bin/update-nginx-blue-green.sh"

# 3. Test Nginx configuration
ssh root@<YOUR_SERVER_IP> "nginx -t && systemctl reload nginx"

# 4. Verify setup
ssh root@<YOUR_SERVER_IP> "/usr/local/bin/update-nginx-blue-green.sh status"
```
