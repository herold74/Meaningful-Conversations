...
async function generatePromptForUser(userId, decryptedProfile, language = 'de', botId = null) {
  try {
    ...
    if (botId === 'dan-client-language' && adaptivePrompt) {
      console.log('🧪 [DPC] Dan: restricted client exact language injection (no blindspots/challenges)');
    }
    ...
    return { 
      ...
      mergeMetadata: {
        ...
        ...(botId === 'dan-client-language' ? { danRestrictedInjection: true } : {})
      }
    };
  } catch (error) {
...
/**
 * DPC/DPFL injection for Dan (client exact language): pacing / courtesy only — no blindspots,
 * challenges, or narrative blindspots/growth (conflicts with verbatim client exact language).
 */
function buildDanCleanLanguageAdaptivePrompt(mergeResult, narrativeProfile, narrativeValidation, language, t) {
  const dan = {
    de: {
      header: '\n\n--- PERSONALISIERTES PROFIL (client exact language — eingeschränkt) ---\n\n',
      intro:
        'Du arbeitest als Dan mit client exact language (Hauptprompt). Die folgenden Profilhinweise wirken **nur** auf Tempo, Ruhe und Höflichkeit im Gespräch — **nicht** auf Inhaltsdeutungen, Ziele oder Entwicklungsthemen.\n\n',
    ...
    en: {
      ...
      intro:
        'You are Dan using client exact language (main prompt). The profile notes below affect **only** pacing, spaciousness, and conversational courtesy — **not** interpretations, goals, or development agendas.\n\n',
      ...
      pacingBullets:
        ...
        '- Still follow: **client exact language questions** using **exact** client wording; at most **one** primary question per message.\n' +
      ...
      formatFooter:
        '**Output:** Natural language; no markdown dividers (---); no strategy meta-comments; no "I understand, your goal is…" paraphrases — at most one brief neutral acknowledgment before the single client exact language question.\n'
    }
  };
