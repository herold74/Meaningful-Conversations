#!/bin/bash
################################################################################
# Deployment Script for Alternative Server (Podman-based)
# Deploys to ssh root@46.224.37.130
################################################################################
...
# Configuration
REMOTE_HOST="root@46.224.37.130"  # Default, can be overridden with --server
REMOTE_DIR="/opt/meaningful-conversations"
...
show_help() {
    echo -e "${GREEN}Alternative Server Deployment for Meaningful Conversations${NC}"
    echo ""
    echo "Usage: ./deploy-alternative.sh [OPTIONS]"
    echo ""
    echo "Options:"
    echo "  -e, --env ENV         Environment: staging (default) or production"
    echo "  -c, --component COMP  Component: all (default), frontend, or backend"
    echo "  --server HOST         Remote server (default: root@46.224.37.130)"
    ...
    echo ""
    echo "Examples:"
    echo "  ${BLUE}./deploy-alternative.sh${NC}                    # Deploy all to staging"
    echo "  ${BLUE}./deploy-alternative.sh -e production${NC}      # Deploy all to production"
    echo "  ${BLUE}./deploy-alternative.sh -c frontend${NC}        # Deploy frontend to staging"
    echo "  ${BLUE}./deploy-alternative.sh --server root@1.2.3.4${NC}  # Deploy to different server"
    echo "  ${BLUE}./deploy-alternative.sh -e production -c frontend${NC}  # Deploy frontend to production"
    echo ""
}
...
if [ -f "$ENV_FILE" ]; then
    ...
    echo -e "${YELLOW}   Create it using: ./setup-alternative-env.sh${NC}"
...
echo -e "${GREEN}╔═══════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  Meaningful Conversations - Alternative Server        ${GREEN}║${NC}"
echo -e "${GREEN}╚═══════════════════════════════════════════════════════╝${NC}"
echo ""
